...
static void equal_size(Monitor *);
...
static int bh, blw = 0, ubutton_w = 0;      /* bar geometry */
...
drawbar(Monitor *m)
{
	...
	unsigned int i, occ = 0, urg = 0, tmp_ubutton_w = 0;
    char buf[50];
	Client *c;
...
tile(Monitor *m)
{
    ...
    // count clients that are tiled
	for (n = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), n++);
	if (n == 0)
		return;

    // nmaster is how many are in master stack
	if (n > m->nmaster)
                    /* window width */
        /*
         * find master width i think, this is the main window on the left
         * mheight is just window height
        */
...
void equal_size(Monitor* mon){
    ...
    if(n_cols > n)n_cols = n;

    FILE* fp = fopen("/home/asher/N_COLS", "w");
    fprintf(fp, "n_cols: %i\n", n_cols);
    fclose(fp);

    // only when perfectly divisible does it look okay
    // need to make sure that width is appropriate
    width = mon->ww/n_cols;
    max_c_per_col = (int)ceil((double)n/(double)n_cols);
    /*
     * x offset max = max width
     * y offset max = max height
    */
    ...
    8 windows, n = 6 is bad
    also 1 window with high n is bad, window is glued
    to the left size with amount of space it would have if windows were full

    width of each is (mon->ww/n_cols)
    n_in_col = MIN(ceil(n/n_cols), remaining)
    height of each is (mon->wh/(n_in_col))
    #endif

    /*crashes when n_cols is too large, 4 i think*/
    /*AHA! do i just need to use HEIGHT?*/
	for (i = 0, c = nexttiled(mon->clients); c; c = nexttiled(c->next), ++i){
        // experimental - test out with 4 windows and N_COLS == 3
        /*partially working, but now n == 4, n_cols == 2 is broken*/
        /*
         * we're at n == 4, i == 2, col_idx == 1, n_cols == 2
         * 4-2 == 2-1
        */
        if(n-i-1 == n_cols-col_idx){
            height = mon->wh;
            max_ht = 1;
        }
        /*if(!heights[i%max_c_per_col])heights[i%max_c_per_col] = ;*/
        /*c_per_col = MIN(ceil(n/n_cols), n_remaining);*/
        /*c_per_col = MAX(1, MIN(max_c_per_col, --n_remaining));*/
        if(!max_ht){
            c_per_col = MAX(1, MIN(max_c_per_col, n-i));
            height = mon->wh/c_per_col;
        }
        /*need to adjust width just like we're doing height*/
        /*n_cols should just be set to n_windows if too high*/
        if(xy_offsets[0] >= mon->ww)xy_offsets[0] = 0;
        if(xy_offsets[1] >= mon->wh){
            // if we're updating height, time to update width/progress to next column
            ++col_idx;
            xy_offsets[0] += width;
            xy_offsets[1] = 0;
        }
        // if we now have a matching number of columns and clients, we can 
        // fit height to screen
        /*damn, this max_ht stuff has the code broken!*/
        /*test with n == 4, col = 2*/
        resize(c, mon->wx+xy_offsets[0], mon->wy+ ((max_ht) ? 0 : xy_offsets[1]), width, height, 0);
        // ah, we should only add to width after we're done with a column!
        /*xy_offsets[0] += width;*/
        xy_offsets[1] += height;
        /*xy_offsets[1] += HEIGHT(c);*/
        /*if(c_per_col == 1)xy_offsets[0] = xy_offsets[1] = 0;*/
        /*
         * resize to wx+startx, wy+starty
         * startx = 0 for first COLUMN
         * starty = 0 for first ROW
         * // width is really width_PER_COLUMN
         * startx = 0+width for second COLUMN
         * starty = 0+height for second ROW
        */
    }
}
